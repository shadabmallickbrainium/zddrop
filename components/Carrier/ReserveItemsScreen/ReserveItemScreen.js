...
export default class ReserveItemScreen extends Component {
  constructor(props) {
    super(props);
  ...
  getReservedItems = async () => {
    ...
    await Apis.getReservedList(data)
      .then(res => {
        if (res[0].success === true) {
          this.setState({ reservedItems: res[0].data })
  ...
  removeReservedItem = async (productId) => {
    this.setState({ refreshing: true })
    const userid = await AsyncStorage.getItem(USER_ID);
    const data = {
      productId: productId,
    ...
    await Apis.removeItemFromReservedList(data)
      .then(res => {
        if (res[0].success === true) {
          this.setState({ refreshing: false })
        }
  ...
  alertFunc = (productId) => {
    Alert.alert("Warning", 'Do you really want to remove this item ?', [{
      text: 'Yes',
      onPress: this.removeReservedItem.bind(this, productId)
    }, {
      text: 'No'
    }])
  }
    ...
    return (
      <ImageBackground
        ...
        <SafeAreaView style={{ flex: 1 }}>
          ...
          {this.state.refreshing ? <View style={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center' }}>
            <ActivityIndicator size='large' color='green' />
          </View> : this.state.searchItems.length === 0 ?
              (this.state.reservedItems.length === 0 ? <View style={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center' }}>
                <Text>No reserved items</Text>
              </View> :
                <ScrollView
                  // refreshControl={<RefreshControl refreshing={this.state.isLoading} onRefresh={this.getReservedItems} />}
                            ...
                            return (
                              <View style={styles.prod_itm} key={item.product_id}>
                                ...
                                <View style={styles.prod_desc}>
                                  ...
                                  <Text style={styles.prod_prc}>${parseInt(item.price).toFixed(2)}</Text>
                                  <Text style={styles.prod_date}>Date : 12.06.2020</Text>
                                  <TouchableOpacity onPress={this.alertFunc.bind(this, item.product_id)}>
                                ...
                                <View style={styles.prod_def_btn}>
                                  <TouchableOpacity>
                                    <View style={styles.def_btn_wrap2} >
                                      <Text style={styles.shipped}>Details</Text>
                ...
                <SearchItemScreen searchItems={this.state.searchItems} navigation={this.props.navigation} backpath='ReserveItemScreen' />
              )}
        </SafeAreaView>
      </ImageBackground>
    );
