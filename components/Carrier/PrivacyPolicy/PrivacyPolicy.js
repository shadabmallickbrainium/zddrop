...
export default class PrivacyPolicy extends Component {
  ...
  async getPrivacy() {
    Hud.showHud();
    await Apis.privacyPolicy()
      .then(async res => {
        if (res.status == 'true') {
          Hud.hideHud();
          if (res.data != null && res.data.length > 0) {
            console.warn('Rwesprivacyyyy', res.data[0].privacyPolicy);
            this.setState({ privacycontent: res.data[0].privacyPolicy });
          }
        } else {
                ...
                <ScrollView
                  ...
                  <View style={{ marginTop: 20, marginBottom: 30, marginLeft: 25, marginRight: 25, padding: 5, paddingBottom: 15, borderRadius: 8, backgroundColor: "white", shadowColor: '#b9b9b9', shadowOpacity: 0.1, elevation: 15 }}>
                    <View style={{ padding: 15 }}>
                      ...
                      }}>{this.state.privacycontent.replace(/(<([^>]+)>)/ig, '').replace(/(&nbsp;)/ig, '')}</Text>
                    </View>
                  </View>
                </ScrollView>
              </View>
